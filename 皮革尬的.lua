...
local FeatureHandlers = {
    ...
    NightVision = {
        ...
        disable = function()
            ...
            if handler.originalLighting.FogEnd then
...
ClickTP = {
    enable = function()
        ...
        -- 桌面端：鼠标点击事件
        local function handleDesktopInput(input, gameProcessed)
        ...
        -- 移动端：触摸点击事件
        local function handleMobileInput(input, gameProcessed)
        ...
        if UserInputService.TouchEnabled then
            Connections.ClickTP = UserInputService.TouchStarted:Connect(handleMobileInput)
            notify("点击传送", "已启用（触摸模式）", 2)
        else
            Connections.ClickTP = UserInputService.InputBegan:Connect(handleDesktopInput)
            notify("点击传送", "已启用（鼠标模式）", 2)
        end
    end,
...
Tabs.Main:Paragraph({
    Title = "设备信息",
    Desc = deviceInfo,
    Image = "rbxassetid://6026663425", -- 通用设备图标
    ImageSize = 42,
    Thumbnail = "rbxassetid://6026663425",
    ThumbnailSize = 0
})
